...
## K188 — Consent-aware conversion intake

Campaign cards now offer **Conversion intake**. Collection is **off by default**. An owner can enable it only for an open Google/Meta campaign with a recognized K187 link and a published page containing a privacy policy. Settings use a revision compare-and-swap; disabling invalidates open measurement contexts. Publishing this release does not enable collection for any owner.

A visitor following that recognized link sees a separate, unchecked, optional advertising measurement choice. Declining does not block the inquiry or grant marketing consent. The fixed `measurement_v1` text names the published brand and Google or Meta; it permits only the link's click identifier and inquiry time, not contact details or message content. Guided review signs the exact choice and context. Contact/request edits and validation redisplay retain the choice. Ordinary/old forms continue to submit without measurement evidence.

Supported URL candidates are one of `gclid`, `gbraid`, `wbraid` for Google or `fbclid` for Meta, 1–512 URL-safe characters. Missing, malformed, repeated or ambiguous matching parameters yield no supported click. Other-provider parameters do not substitute for the selected provider. This is syntactic validation only: a fabricated/copied identifier is not proof of an ad click. There is no provider call, SDK, pixel, third-party script, IP/user-agent capture, name/email/phone hash or enhanced-conversion upload.

Before opt-in, the candidate exists only during request handling and in a short-lived AES-256-GCM hidden form envelope. It is bound to the exact HMAC/cookie form token via authenticated additional data; its key is purpose-derived from the existing form secret. It is not stored in a new cookie or database row. It cannot move between forms, campaigns or published page versions. Public responses now use `Referrer-Policy: no-referrer` so links and asset requests do not forward the landing URL. Incoming URL parameters can still exist in browser history and infrastructure access logs; this release does not change hosting log retention.

The new service-only invoker RPC `korlix_funnel_measurement_v1(uuid,text,uuid,jsonb)` uses the existing funnel transaction lock and calls the unchanged K187 attribution/capture RPC. CRM contact, inquiry, follow-up, link evidence and measurement receipt commit together. On decline, click fields are null even if a candidate was supplied. On opt-in, an immutable receipt stores the click identifier, first-observed time, stable event UUID, exact disclosure/privacy URL, published page version, settings revision and inquiry capture timestamp. Event `inquiry_submitted` never means a booking or sale. No historical backfill; retries preserve the original consent and event. Deleted inquiries cascade their receipts and identifiers, and cleanup tombstones prevent replay resurrection. Retention follows inquiry retention; there is no new automatic purge schedule.

Local states are `declined`, `missing_click` (allowed but no supported identifier), and `awaiting_setup` (allowed with a syntactically valid identifier). **Awaiting setup is not a delivery queue, provider receipt or verified conversion.** No destination/account mapping or upload permission is inferred from a local campaign. Uploads require future explicit provider setup and acceptance work. Disabling retains earlier receipts; future delivery must recheck current permissions/consent/eligibility and expiration before uploading anything.

Owner routes `/api/funnels/:id/campaigns/:campaign_id/measurement`:

- `GET ?days=7|30|90` (default 30), 30/owner/minute: current settings and aggregate daily receipt counts. No raw identifiers, lead/event IDs or contact data are returned.
- `POST /settings`, exactly `{days: 7|30|90, enabled: boolean, expected_revision: uuid|null}`, 5/owner/minute. The returned current revision is required for subsequent changes. A lost response should be reconciled with GET before another save.

Both enforce current Enterprise ownership and no-store. Reports include today in UTC and reconcile three mutually exclusive states to total receipts. The UI rejects mismatched scope, date gaps, inconsistent totals, unsupported event/policy or misleading provider delivery flags; errors/access loss/workspace changes clear private results and discard late responses. Enabling/disabling is explicit and disabling remains available while collection is suspended.

Migration `20260924020959_funnel_conversion_intake.sql` adds RLS-protected policy-free settings and receipt tables. It explicitly revokes hosted default privileges before granting service_role SELECT/INSERT/UPDATE on settings and SELECT/INSERT on immutable receipts. No browser grants or existing RPC replacements. Apply migration before backend, then deploy frontend. For rollback, restore frontend/backend K187 while keeping the additive tables and evidence; document the intake gap rather than backfilling it.

Verification: actual PGlite SQL and Express tests cover hosted grants, ownership/tiers, strict inputs, revision races, default-off, opt-in/decline, malformed and duplicate candidates, envelope tampering/transfer, guided review edits, disable/re-enable, lost responses/retries, atomic rollback, no backfill, cleanup/tombstones, UTC windows and page/entitlement changes. Flutter validates exact receipt contracts, controls, errors, access/scope loss and real-font 1400/390/320px layouts (1.3 scale at 320). This establishes local behavior, not owner A–G or live-provider acceptance.

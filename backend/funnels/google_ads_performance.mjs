...
export async function readGooglePerformance(ads,token,account,root,range) {
  ...
  const query=`SELECT customer.id, customer.currency_code, customer.time_zone, segments.date, metrics.cost_micros, metrics.impressions, metrics.clicks FROM customer WHERE segments.date BETWEEN '${range.from}' AND '${range.to}' ORDER BY segments.date DESC LIMIT 91`;
  ...
  for(let page=0;page<3;page++){
    ...
    const data=result.results??[];if(data.length>range.days)unreadable();
    for(const row of data){
      ...
      const day=row.segments?.date;date(day);if(day<range.from||day>range.to||dates.has(day))unreadable();dates.add(day);
      ...
      if(dates.size>range.days||totalSpend>MAX_SPEND||totalImpressions>MAX_COUNT||totalClicks>MAX_COUNT)unreadable();
      rows.push({date:day,spend:money(spend),impressions:Number(impressions),clicks:Number(clicks)});
    }
    if(result.nextPageToken===undefined||result.nextPageToken==='')return {rows:rows.sort((a,b)=>b.date.localeCompare(a.date)),totals:{spend:money(totalSpend),impressions:Number(totalImpressions),clicks:Number(totalClicks)},reported_days:rows.length};
    pageToken=result.nextPageToken;
    if(typeof pageToken!=='string'||!pageToken||pageToken.length>4000||pages.has(pageToken))unreadable();pages.add(pageToken);
  }
  fail('Google reporting exceeded the page limit. Try a shorter period.',503);
}

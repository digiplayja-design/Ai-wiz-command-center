...
export function registerGoogleKeywords(app,{base,owner,database,publicBase}) {
  ...
  const run=action=>owner(async(q,r,u)=>{
    ...
    const data={campaign_id:uuid(q.params.campaign_id),public_base:root.href.replace(/\/$/,''),...(action==='read'?{}:action==='save'?googleKeywordsInput(q.body):googleKeywordsReviewInput(action,q.body))};

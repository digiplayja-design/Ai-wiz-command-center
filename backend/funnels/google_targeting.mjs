...
import {googleLocationsValid,googleLocationSearchInput,searchGoogleLocations} from './google_locations.mjs';
...
export function googleTargetingAssets(v) {
  ...
  if(!v||typeof v!=='object'||Array.isArray(v)||keys.some(k=>!Object.hasOwn(v,k))||Object.keys(v).some(k=>!keys.includes(k)&&k!=='proximities'&&k!=='geo_locations'))fail('Choose countries, cities/regions or radius areas, content languages, reach and a bidding preference.');
  ...
  if(Object.hasOwn(v,'geo_locations')&&(!googleLocationsValid(v.geo_locations)||v.countries.length||Object.hasOwn(v,'proximities')||v.geo_locations.some(x=>v.excluded_countries.includes(x.country))))fail('Choose up to 20 listed cities or regions outside the excluded countries, with no country or radius targets.');
  return v;
}
...
export function registerGoogleTargeting(app,{base,owner,database,publicBase}) {
  ...
  const run=action=>owner(async(q,r,u)=>{
    const lookup=action==='locations'?googleLocationSearchInput(q.query):null;
    if(!lookup&&Object.keys(q.query).length)fail('Open the targeting draft without extra parameters.');
    ...
    const data={campaign_id:uuid(q.params.campaign_id),public_base:root.href.replace(/\/$/,''),...(['read','locations'].includes(action)?{}:action==='save'?googleTargetingInput(q.body):googleTargetingReviewInput(action,q.body))};
    const {data:result,error}=await database.rpc('korlix_funnel_google_targeting_v1',{p_actor:u,p_action:lookup?'read':action,p_funnel:uuid(q.params.id),p_data:data});
    ...
    if(lookup){if(result?.locations_supported!==true)fail('City and region search is temporarily unavailable.',503);r.json(searchGoogleLocations(lookup));}
    else r.json(result);
  },{ratePrefix:'google-targeting:',max:30});
  app.get(route+'/locations',run('locations'));

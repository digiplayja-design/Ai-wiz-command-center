...
export function registerFunnels(app,{database,requireUser,store,followups,campaignStore,metaPreparationStore,googlePreparationStore,generateAdCopy,metaStore,metaProvider,googleAdsStore,googleAdsProvider,googleUploadProvider,rehearsalStore,imageStore,loadAgentProfile,generate=generateFunnel,environment=process.env,now=Date.now,autoStartScheduler=false,logger=console}={}) {
  ...
  registerGoogleUploadAccess(app,{base,owner,limit,database,googleAdsProvider,googleUploadProvider,environment,now});

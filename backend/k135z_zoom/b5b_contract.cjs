...
function rtmsUrls(v) {
  text(v,4096); const values=v.split(','); need(values.length<=4,'ZOOM_RTMS_ENDPOINT_INVALID');
  for(const value of values) {
    let u;try{u=new URL(value);}catch{need(false,'ZOOM_RTMS_ENDPOINT_INVALID');}
    need(value===value.trim() && u.protocol==='wss:' && !u.username && !u.password && !u.hash &&
      (!u.port||u.port==='443') && /^(?:[a-z0-9-]+\.)+zoom\.us$/i.test(u.hostname),'ZOOM_RTMS_ENDPOINT_INVALID');
  }
  return v;
}
function rtmsSource(v) {
  keys(v,['operatorId','originalHost','serverUrls']);text(v.operatorId);
  need(typeof v.originalHost==='boolean','ZOOM_RTMS_HOST_INVALID');rtmsUrls(v.serverUrls);return structuredClone(v);
}
function captureQuery(v) {
  keys(v,['key','meetingUuid','streamId']);identityFromKey(v.key);text(v.meetingUuid,512);
  if(v.streamId!==null)text(v.streamId,512);return structuredClone(v);
}
function captureSource(v) {
  keys(v,['meetingUuid','streamId','serverUrls']);text(v.meetingUuid,512);text(v.streamId,512);rtmsUrls(v.serverUrls);
  return structuredClone(v);
}
function eventPlan(verified) {
  ...
  if (event === 'app_deauthorized') {
    ...
    const account = p?.account_id === undefined && event !== 'meeting.rtms_started' ? null : text(p?.account_id);
    for (const [current, legacy] of [[p?.meeting_uuid, o?.uuid], [p?.rtms_stream_id, o?.rtms_stream_id]])
      need(current === undefined || legacy === undefined || current === legacy, 'ZOOM_EVENT_BINDING_CONFLICT');
    const uuid = text(p?.meeting_uuid === undefined ? o?.uuid : p.meeting_uuid, 512);
    const stream = text(p?.rtms_stream_id === undefined ? o?.rtms_stream_id : p.rtms_stream_id, 512);
    if (account === null) {
      mutation = {kind:'terminal',meetingUuid:uuid,streamId:stream,status:event.slice('meeting.rtms_'.length)};
    } else {
    ...
    if(event==='meeting.rtms_started' && (p.operator_id!==undefined || p.is_original_host!==undefined))
      mutation.source=rtmsSource({operatorId:p.operator_id,originalHost:p.is_original_host,serverUrls:p.server_urls});
    }
  }
...
function validatePlan(v) {
  ...
  if (m.kind === 'none') keys(m,['kind']);
    ...
    keys(m,['kind','meetingUuid','streamId','status']); text(m.meetingUuid,512); text(m.streamId,512);
    need(['stopped','interrupted'].includes(m.status) && v.event === 'meeting.rtms_' + m.status,'ZOOM_EVENT_MUTATION_INVALID');
  }
  else if (m.kind === 'session') { keys(m,['kind','key','record'],['source']); sessionRecord(m.key,m.record); if(Object.hasOwn(m,'source')) {need(m.record.status==='started','ZOOM_EVENT_MUTATION_INVALID');rtmsSource(m.source);} need(v.event === 'meeting.rtms_' + m.record.status && v.eventTs === m.record.eventTs,'ZOOM_EVENT_MUTATION_INVALID'); }
  else need(false, 'ZOOM_EVENT_MUTATION_INVALID');
  return structuredClone(v);
}
module.exports = {rtmsUrls,rtmsSource,captureQuery,captureSource,K135zZoomError, need, keys, text, time, identity, identityKey, identityFromKey,

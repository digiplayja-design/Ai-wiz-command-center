...
class SupabaseZoomRepository {
  ...
  async rpc(operation, payload) {
    let result;
    try { result = await this.client.rpc(RPC, {operation, payload}); }
...
class MemoryZoomRepository {
  constructor() { this.kind='memory-offline-only'; this.oauthStates=new Map(); this.usedOAuthStates=new Set(); this.connections=new Map(); this.webhookEvents=new Map(); this.rtmsSessions=new Map(); }
  ...
  async upsertRtmsSession(k,r) {
    ...
    if(Object.hasOwn(r,'zoomAccountId')) return this.session(k,C.sessionRecord(k,r));
  ...
  async applyWebhookEvent(input) {
    ...
    if(p.mutation.kind==='session') this.session(p.mutation.key,p.mutation.record);

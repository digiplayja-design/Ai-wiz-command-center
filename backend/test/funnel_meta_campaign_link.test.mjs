...
import {metaReportRange} from '../funnels/meta_performance.mjs';
import {linkedMetaRows,metaCampaignChoiceProof,metaCampaignChoiceProofValid} from '../funnels/meta_campaign_link.mjs';
const owner=randomUUID(),other=randomUUID(),basic=randomUUID();
const env={KORLIX_META_ENABLED:'true',KORLIX_META_APP_ID:'1234567',KORLIX_META_APP_SECRET:'fixture-link-secret-not-real',KORLIX_META_LOGIN_CONFIG_ID:'7654321',KORLIX_META_TOKEN_KEY:Buffer.alloc(32,7).toString('base64'),KORLIX_META_REDIRECT_URI:'https://example.com/api/funnels/meta/callback'};
const config=metaConfiguration(env),defaultAccount={id:'act_123',name:'Growth account',currency:'KWD',timezone:'America/New_York',status:1};
...
const providerReport=()=>({rows:[{campaign_id:'456',campaign_name:'Same name',spend:'0.123456',clicks:3,impressions:20},{campaign_id:'789',campaign_name:'Same name',spend:'2.00',clicks:7,impressions:80}],totals:{spend:'2.123456',clicks:10,impressions:100},reported_campaigns:2});
let db,server,base,f,c,account,clock=Date.now(),providerCalls=0,reportImpl,accountImpl,handler;
...
const link=(action='read',data={},actor=owner)=>rpc('korlix_funnel_meta_campaign_link_v1',{p_actor:actor,p_action:action,p_funnel:f.id,p_data:{campaign_id:c.id,configured:true,config_hash:config.hash,...data}});
const request=(suffix='',body=null,actor=owner,campaignId=c.id)=>fetch(`${base}/api/funnels/${f.id}/campaigns/${campaignId}/meta-link${suffix}`,{method:body===null?'GET':'POST',headers:{authorization:actor,'content-type':'application/json'},...(body===null?{}:{body:JSON.stringify(body)})});
async function connect(){const binding=randomUUID();await db.query('insert into korlix_meta_connections(user_id,binding_id,config_hash,meta_user_id,sealed,expires_at,accounts,selected_account,refreshed_at) values($1,$2,$3,$4,$5,now()+interval \'1 day\',$6,$7,now())',[owner,binding,config.hash,'123456',JSON.stringify(tokenCipher(config.key).seal('private-fixture-token',`korlix-meta:${owner}:${binding}`)),JSON.stringify([account]),account.id]);}
...
test.before(async()=>{
 ...
 const app=express();app.use(express.json());handler=registerFunnels(app,{database,requireUser:async q=>[owner,other,basic].includes(q.headers.authorization)?{id:q.headers.authorization}:null,environment:env,now:()=>clock,metaProvider:{account:async(...args)=>{providerCalls++;return accountImpl(...args);},campaignInsights:async(...args)=>{providerCalls++;return reportImpl(...args);}}});
 server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));base='http://127.0.0.1:'+server.address().port;
});
test.beforeEach(async()=>{clock=Date.now()+60000*counter++;await db.exec('delete from korlix_funnels;delete from korlix_meta_connections;');await db.query("update user_profiles set tier='enterprise' where id=$1",[owner]);account={...defaultAccount};f=await funnel('create',{name:'Services',slug:'services',document:doc});c=await campaign('create',plan);await connect();providerCalls=0;accountImpl=async()=>account;reportImpl=async()=>providerReport();});
let counter=1;
test.after(async()=>{handler.close();server.closeAllConnections();await new Promise(r=>server.close(r));await db.close();});
test('Local read is private, owner/tier/platform-scoped and never calls Meta or leaks credentials',async()=>{
 ...
 await db.query("update korlix_funnel_campaigns set platform='google' where id=$1",[c.id]);assert.equal((await request()).status,400);
 for(const role of ['anon','authenticated']){await db.exec('reset role;set role '+role);await assert.rejects(db.query('select * from korlix_funnel_meta_campaign_links'),/permission denied/);await assert.rejects(link(),/permission denied/);await db.exec('reset role;set role service_role');}
 const p=(await db.query("select md5(prosrc) body_hash,prosecdef,proconfig,(select relrowsecurity from pg_class where oid='korlix_funnel_meta_campaign_links'::regclass) rls from pg_proc where oid='korlix_funnel_meta_campaign_link_v1(uuid,text,uuid,jsonb)'::regprocedure")).rows[0];assert.equal(p.prosecdef,false);assert.equal(p.rls,true);assert.deepEqual(p.proconfig,['search_path=public, pg_temp']);await writeFile('/tmp/k179-function-hashes.json',JSON.stringify(p,null,2));
});
test('Choices use provider IDs even for identical names; saving only changes the link and never stores a receipt/metrics',async()=>{
 ...
 const row=(await db.query('select to_jsonb(x) r from korlix_funnel_meta_campaign_links x')).rows[0].r;
...
test('Performance contains only the linked row with exact decimals and matching local tagged inquiries',async()=>{
 await saveLink();const range=metaReportRange(7,account.timezone,clock);
...
test('No Meta row is not a zero result; an explicit zero row remains reportable',async()=>{
 ...
 reportImpl=async()=>({rows:[{campaign_id:'456',campaign_name:'Renamed by Meta',spend:'0.00',clicks:0,impressions:0}],totals:{spend:'0.00',clicks:0,impressions:0},reported_campaigns:1});r=await(await performance()).json();assert.equal(r.row.spend,'0.00');assert.equal(r.row.campaign_name,'Renamed by Meta');assert.equal(r.link.provider_campaign_name,'Same name');
});
test('Provider account changes and malformed adapter results fail before any receipt is issued',async()=>{
 for(const patch of [{id:'act_999'},{currency:'USD'},{timezone:'UTC'},{status:2}]){accountImpl=async()=>({...account,...patch});const d=await link();assert.equal((await request(`/campaigns?days=7&fingerprint=${d.fingerprint}`)).status,409);}
...
test('Disconnect, downgrade, plan edit and relink during provider work suppress late output',async()=>{
 for(const change of ['disconnect','downgrade','plan','clear']){
  clock+=60000;await db.exec('delete from korlix_meta_connections;delete from korlix_funnel_meta_campaign_links;');await db.query("update user_profiles set tier='enterprise' where id=$1",[owner]);await connect();reportImpl=async()=>providerReport();await saveLink();
  reportImpl=async()=>{if(change==='disconnect')await db.exec('delete from korlix_meta_connections');else if(change==='downgrade')await db.query("update user_profiles set tier='basic' where id=$1",[owner]);else if(change==='plan')c=await campaign('save',{...plan,name:'New plan'});else{const d=await link();await link('clear',{version:d.version,fingerprint:d.fingerprint,confirmed:true});}return providerReport();};
...
test('Revoked provider access marks reconnect without leaking remote details',async()=>{
 reportImpl=async()=>{throw new MetaAccessError();};const d=await link(),r=await request(`/campaigns?days=7&fingerprint=${d.fingerprint}`);assert.equal(r.status,409);assert.equal((await link()).lookup_ready,false);assert.equal((await db.query('select needs_reconnect from korlix_meta_connections')).rows[0].needs_reconnect,true);
});
test('Connection reselection/configuration/binding changes stale links; clear works after disconnect and on archive',async()=>{
 let d=await saveLink();await db.query("update korlix_meta_connections set selected_account=null,version=nextval('korlix_meta_version_seq')");assert.equal((await link()).link_current,false);
 await db.exec('delete from korlix_meta_connections');await connect();assert.equal((await link()).link_current,false);
 ...
 await db.exec('delete from korlix_meta_connections');assert.equal((await link()).link,null);
});
test('SQL rejects stale/unconfirmed/forged shapes and out-of-period measurement; no arbitrary range is accepted',async()=>{
 ...
 const range=metaReportRange(7,account.timezone,clock);
...
test('New reporting routes share the existing Meta report quota and reject unconfigured access',async()=>{
 ...
 const conn=(await db.query('select version from korlix_meta_connections')).rows[0];assert.equal((await fetch(`${base}/api/funnels/meta/campaign-performance?days=7&version=${conn.version}&account_id=${account.id}`,{headers:{authorization:owner}})).status,429);
...
test('Receipt domain binds every identity and expires; malformed metrics never receive signatures',()=>{
 ...
 const proof=metaCampaignChoiceProof('secret',scope,row,time);assert(metaCampaignChoiceProofValid('secret',scope,row,proof,time));
 for(const field of ['actor','funnel','campaign','fingerprint'])assert(!metaCampaignChoiceProofValid('secret',{...scope,[field]:'other'},row,proof,time));
 assert(!metaCampaignChoiceProofValid('other',scope,row,proof,time));assert(!metaCampaignChoiceProofValid('secret',scope,row,proof,time+1800000));
 for(const change of [{spend:'NaN'},{spend:'-1'},{clicks:9007199254740992},{campaign_id:'../x'},{campaign_name:'\0'}])assert.throws(()=>linkedMetaRows({...providerReport(),rows:[{...providerReport().rows[0],...change},providerReport().rows[1]]}));
});

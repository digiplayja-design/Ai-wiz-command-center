...
import {metaPreparationInput} from '../funnels/meta_preparation.mjs';
...
const req=(suffix='',body=null,actor=owner,ids={})=>fetch(`${base}/api/funnels/${ids.funnel??f.id}/campaigns/${ids.campaign??c.id}/meta-setup${suffix}`,{method:body===null?'GET':'POST',headers:{Authorization:actor,'Content-Type':'application/json'},...(body===null?{}:{body:JSON.stringify(body)})});
...
test('Setup preview is owner scoped, credential free, read only and uses the server campaign destination',async()=>{
 const r=await req();assert.equal(r.status,200);assert.equal(r.headers.get('cache-control'),'no-store');const body=await r.json();assert.equal(body.ready_for_review,true);assert.equal(body.review_current,false);assert.equal(body.version,0);assert.equal(body.ad_publishing_ready,false);assert.equal(body.current_snapshot.campaign.planned_total_cents,35000);assert.equal(body.current_snapshot.meta.page.id,page.id);assert.equal(body.current_snapshot.landing_page.destination,`https://example.com/f/services?utm_source=facebook&utm_medium=paid&utm_campaign=k143_${c.id.replaceAll('-','')}`);
 for(const secret of ['sealed','config_hash','meta_user_id','binding_id','private-fixture-token'])assert(!JSON.stringify(body).includes(secret));assert.equal((await db.query('select count(*)::int n from korlix_funnel_meta_preparations')).rows[0].n,0);assert.equal(providerCalls,0);
});
test('Real HTTP ownership and private SQL privileges deny other owners and browser roles',async()=>{
 ...
 for(const role of ['anon','authenticated']){await db.exec('reset role;set role '+role);await assert.rejects(db.query('select * from korlix_funnel_meta_preparations'),/permission denied/);await assert.rejects(setup(),/permission denied/);await db.exec('reset role;set role service_role');}
 const schema=(await db.query("select p.prosecdef,(select relrowsecurity from pg_class where oid='korlix_funnel_meta_preparations'::regclass) rls from pg_proc p where p.oid='korlix_funnel_meta_preparation_v1(uuid,text,uuid,jsonb)'::regprocedure")).rows[0];assert.equal(schema.prosecdef,false);assert.equal(schema.rls,true);
});
test('Review and clear use exact versions and preserve campaign, connection and reporting data',async()=>{
 const before=(await db.query('select to_jsonb(c) c from korlix_meta_connections c')).rows;const campaignBefore=(await db.query('select to_jsonb(c) c from korlix_funnel_campaigns c')).rows;
 ...
 assert.deepEqual((await db.query('select to_jsonb(c) c from korlix_meta_connections c')).rows,before);assert.deepEqual((await db.query('select to_jsonb(c) c from korlix_funnel_campaigns c')).rows,campaignBefore);assert.equal(providerCalls,0);
});
test('Changed campaign, published content and Meta identities invalidate saved reviews and stale submissions',async()=>{
 ...
 c=await campaign('review',{confirmed:true});const fresh=await saveReview();await db.query('update korlix_meta_connections set selected_page=null where user_id=$1',[owner]);r=await setup();assert.equal(r.review_current,false);assert.equal(r.checks.page_selected,false);
...
test('Configuration, expired access, missing Page and non-USD or inactive accounts block review without converting amounts',async()=>{
 const saved=await saveReview();let r=await setup('read',{configured:false});assert.equal(r.checks.meta_configured,false);assert.equal(r.review_current,false);
 r=await setup('read',{config_hash:'different'});assert.equal(r.checks.meta_connected,false);assert.equal(r.review_current,false);
 await db.query("update korlix_meta_connections set expires_at=now()-interval '1 second' where user_id=$1",[owner]);r=await setup();assert.equal(r.ready_for_review,false);await assert.rejects(setup('review',{version:saved.version,fingerprint:r.fingerprint,confirmed:true}),/Complete the setup/);
 await db.query("update korlix_meta_connections set expires_at=now()+interval '1 day',accounts=$2 where user_id=$1",[owner,JSON.stringify([{...account,currency:'JMD',status:2}])]);r=await setup();assert.equal(r.checks.currency_supported,false);assert.equal(r.checks.account_active,false);assert.equal(r.current_snapshot.campaign.currency,'USD');assert.equal(r.current_snapshot.campaign.daily_cents,2500);
});
test('Disconnect and reconnect cannot reuse a saved Meta setup review',async()=>{
 const saved=await saveReview();await db.query('delete from korlix_meta_connections where user_id=$1',[owner]);let r=await setup();assert.equal(r.checks.meta_connected,false);assert.equal(r.review_current,false);assert.deepEqual(r.reviewed_snapshot,saved.reviewed_snapshot);
...
test('Exact HTTP schemas reject injected destinations, readiness, query fields, and unconfirmed mutations',async()=>{
 ...
 assert.equal((await req('/clear',{version:0})).status,400);await db.query("update korlix_funnel_campaigns set platform='google' where id=$1",[c.id]);assert.equal((await req()).status,400);assert.equal(providerCalls,0);
});
...
test('Setup routes share an owner rate limit and deletion cascades only local review data',async()=>{
 ...
 await db.query('delete from korlix_funnels where id=$1',[f.id]);assert.equal((await db.query('select count(*)::int n from korlix_funnel_meta_preparations')).rows[0].n,0);assert.equal((await db.query('select count(*)::int n from korlix_meta_connections')).rows[0].n,1);
});
test('Preparation input accepts version zero only for an unsaved review and rejects unsafe integers',()=>{
 assert.deepEqual(metaPreparationInput('review',{version:0,fingerprint:'a'.repeat(64),confirmed:true}),{version:0,fingerprint:'a'.repeat(64),confirmed:true});
 for(const version of [-1,1.5,Infinity,2147483648,null])assert.throws(()=>metaPreparationInput('clear',{version,confirmed:true}));
});

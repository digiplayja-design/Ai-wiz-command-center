...
function g6kEvent(status='stopped',fields={},ts=NOW) {
  return {body:{event:'meeting.rtms_'+status,event_ts:ts,payload:{meeting_uuid:'g6k-meeting',rtms_stream_id:'g6k-stream',...fields}}};
}
test('Gate6K documented accountless Stop and Interrupted produce exact terminal plans',()=>{
  for(const status of ['stopped','interrupted']) {
    const p=C.eventPlan(g6kEvent(status));
    assert.deepEqual(p.mutation,{kind:'terminal',meetingUuid:'g6k-meeting',streamId:'g6k-stream',status});
    assert.equal(JSON.stringify(p).includes('zoomAccountId'),false);
  }
});
test('Gate6K Start still requires account and malformed terminal bindings are rejected',()=>{
  assert.throws(()=>C.eventPlan(g6kEvent('started')));
  for(const fields of [{account_id:null},{account_id:''},{meeting_uuid:''},{rtms_stream_id:7},
    {object:{uuid:'conflicting',rtms_stream_id:'g6k-stream'}}]) assert.throws(()=>C.eventPlan(g6kEvent('stopped',fields)));
  const p=C.eventPlan(g6kEvent());
  for(const mutation of [{...p.mutation,status:'started'},{...p.mutation,zoomAccountId:'invented'},
    {...p.mutation,meetingUuid:'\n'}]) assert.throws(()=>C.validatePlan({...p,mutation}));
});
test('Gate6K accountless terminal closes only the exact meeting and stream',async()=>{
  const r=new MemoryZoomRepository(),plans=[];
  for(const fields of [{},{meeting_uuid:'other-meeting'},{rtms_stream_id:'other-stream'}]) {
    const p=C.eventPlan(g6kEvent('started',{account_id:'zoom-account',...fields}));plans.push(p);await r.applyWebhookEvent(p);
  }
  await r.applyWebhookEvent(C.eventPlan(g6kEvent()));
  assert.deepEqual(await Promise.all(plans.map(async p=>(await r.getRtmsSession(p.mutation.key)).status)),['stopped','started','started']);
});
test('Gate6K terminal before Start blocks delayed and newer Start for the same stream',async()=>{
  for(const status of ['stopped','interrupted']) {
    const r=new MemoryZoomRepository();await r.applyWebhookEvent(C.eventPlan(g6kEvent(status)));
    for(const ts of [NOW-1,NOW+1]) {
      const p=C.eventPlan(g6kEvent('started',{account_id:'zoom-account'},ts));await r.applyWebhookEvent(p);
      assert.equal((await r.getRtmsSession(p.mutation.key)).status,status);
    }
  }
});
test('Gate6K old terminal time still stops a newer Start and Stop dominates Interrupted',async()=>{
  const r=new MemoryZoomRepository(),p=C.eventPlan(g6kEvent('started',{account_id:'zoom-account'},NOW+20));
  await r.applyWebhookEvent(p);await r.applyWebhookEvent(C.eventPlan(g6kEvent('stopped',{},NOW)));
  await r.applyWebhookEvent(C.eventPlan(g6kEvent('interrupted',{},NOW+30)));
  const saved=await r.getRtmsSession(p.mutation.key);assert.equal(saved.status,'stopped');assert.equal(saved.eventTs,NOW+30);
});
test('Gate6K duplicate and conflicting terminal IDs retain atomic deduplication',async()=>{
  const r=new MemoryZoomRepository(),e=g6kEvent();e.body.event_id='terminal-one';const p=C.eventPlan(e);
  assert.equal((await r.applyWebhookEvent(p)).accepted,true);assert.equal((await r.applyWebhookEvent(p)).duplicate,true);
  e.body.payload.rtms_stream_id='another';await assert.rejects(r.applyWebhookEvent(C.eventPlan(e)),{code:'ZOOM_EVENT_ID_CONFLICT'});
  assert.equal(r.rtmsTerminals.size,1);
});
test('Gate6K signed accountless webhook succeeds without fabricating an account',async()=>{
  const f=fixture(),v=new ZoomWebhookVerifier({secret:'fixture-webhook',clock:()=>NOW}),res=response();
  const e=g6kEvent('stopped',{stop_reason:'host_stopped'});
  await createK135zZoomHandlers({...f,webhookVerifier:v}).webhook(signed(e.body,v),res);
  assert.equal(res.statusCode,200);assert.equal(res.body.accepted,true);assert.equal(f.repository.rtmsTerminals.size,1);
  assert.equal(f.repository.rtmsSessions.size,0);
});
test('Gate6K altered signed Stop has no terminal or event effect',async()=>{
  const f=fixture(),v=new ZoomWebhookVerifier({secret:'fixture-webhook',clock:()=>NOW}),res=response(),req=signed(g6kEvent().body,v);
  req.rawBody=req.rawBody.replace('g6k-stream','wrong-stream');
  await createK135zZoomHandlers({...f,webhookVerifier:v}).webhook(req,res);
  assert.equal(res.statusCode,401);assert.equal(f.repository.rtmsTerminals.size,0);assert.equal(f.repository.webhookEvents.size,0);
});
test('Gate6K supplied-account terminal retains account isolation',async()=>{
  const r=new MemoryZoomRepository(),plans=[];
  for(const account_id of ['account-a','account-b']) {
    const p=C.eventPlan(g6kEvent('started',{account_id}));plans.push(p);await r.applyWebhookEvent(p);
  }
  await r.applyWebhookEvent(C.eventPlan(g6kEvent('stopped',{account_id:'account-a'})));
  assert.deepEqual(await Promise.all(plans.map(async p=>(await r.getRtmsSession(p.mutation.key)).status)),['stopped','started']);
});
test('Gate6K validated terminal plan passes through service RPC with no media credentials',async()=>{
  const p=C.eventPlan(g6kEvent('interrupted',{server_urls:'wss://never-persist.zoom.us',signature:'never-persist'}));let sent;
  const r=new SupabaseZoomRepository({client:{async rpc(name,args){sent={name,args};return {data:{accepted:true,duplicate:false,deletedConnections:0}};}}});
  await r.applyWebhookEvent(p);assert.equal(sent.name,RPC);assert.equal(sent.args.operation,'event_apply');
  assert.deepEqual(sent.args.payload,p);assert.equal(JSON.stringify(sent).includes('never-persist'),false);
});

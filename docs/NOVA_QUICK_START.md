...
**Silence Nova / Stop Nova** immediately silences this page and cancels its reply request. **Stop listening** also stops capture. The connection options retain a listening-only start. Unsupported browsers retain listening controls. Switching to another tab or app no longer automatically pauses an enabled Nova conversation: replies and waiting speech continue while the browser permits audio and timers. If the browser suspends audio or the capture lease expires, Nova preserves the existing opt-in, discards interrupted/stale turns, revalidates the same session on return, and shows a resume button if the browser requires a new audio gesture. Stop, Pause, revocation, another meeting, sign-out, and leaving the Co-Pilot route do not receive this automatic recovery.

Keep the KORLIX tab open. On phones and tablets, keep it visible for reliable replies. Background operation is best effort; a browser can freeze or discard the page, and this frontend change does not make Nova independent of the browser. No hidden audio loop, wake-lock workaround, lease extension, or automatic reconsent to a changed session is used.
...
- Stop, actual audio suspension, changed permissions, changed meetings, and stale reply completions retain session/epoch checks. Merely hiding the web page keeps the explicitly enabled conversation active while the browser allows it. Captions heard while replying or during the existing four-second echo cooldown are not queued for later replies.

...
enum CampaignSetupPlatform { meta, google }

const googleSetupChecks = <String, (String, String)>{
  'page_published': (
    'Landing page published',
    'Publish the funnel page before reviewing this setup.',
  ),
  ...
  'google_configured': (
    'Google Ads platform setup available',
    'KORLIX Google Ads activation is still needed.',
  ),
  'google_connected': (
    'Google Ads connection current',
    'Connect or reconnect Google Ads in the Ads workspace.',
  ),
  'access_context': (
    'Google access account matches',
    'Refresh your access accounts and select the correct direct account or manager.',
  ),
  'account_selected': (
    'Ad account selected',
    'Select an advertising account in the Google Ads connection card.',
  ),
  'account_active': (
    'Selected ad account active',
    'Refresh Google accounts and choose an enabled advertising account, rather than a manager account.',
  ),
  'currency_supported': (
    'Account matches the USD plan',
    'This plan uses USD. Choose a USD ad account; no currency conversion is applied.',
  ...
  'production_account': (
    'Production account selected',
    'This account is for testing. Select a production advertising account for this setup review.',
  ),
};
Map<String, (String, String)> setupChecks(CampaignSetupPlatform platform) =>
...
bool _same(dynamic a, dynamic b) => a is Map && b is Map
    ...
    : a == b;
Map<String, dynamic> validateCampaignPreparation(
  Map<String, dynamic> r,
  String funnel,
  String campaign, {
  required CampaignSetupPlatform platform,
}) {
  final google = platform == CampaignSetupPlatform.google;
  final identityKey = google ? 'google_ads' : 'meta';
  final expectedChecks = setupChecks(platform);
  final label = google ? 'Google Ads' : 'Meta';
  ...
  bool snapshot(dynamic v) {
    if (v is! Map ||
        ...
        v[identityKey] is! Map) {
      return false;
    }
    final c = v['campaign'], p = v['landing_page'], m = v[identityKey];
    if (c['id'] != campaign ||
        !text(c['name'], 100) ||
        !text(c['headline'], 180) ||
        !text(c['body'], 2000) ||
    ...
    if (url == null ||
        ...
        !_same(url.queryParameters, {
          'utm_source': google ? 'google' : 'facebook',
          'utm_medium': 'paid',
    ...
    if ((m['connection_version'] != null &&
        ...
        (!google && !time(m['pages_refreshed_at']))) {
      return false;
    }
    bool googleId(dynamic value) =>
        value is String && RegExp(r'^\d{10}$').hasMatch(value);
    ...
    if (google) {
      if ((m['root_id'] != null && !googleId(m['root_id'])) ||
          (m['root_name'] != null && !text(m['root_name'], 200)) ||
          (m['login_customer_id'] != null &&
              !googleId(m['login_customer_id']))) {
        return false;
      }
    } else if (page != null &&
        (page is! Map ||
  ...
  bool selectedIdentity(dynamic s) {
    final m = s[identityKey];
    return m['connection_version'] != null &&
        m['account'] != null &&
        m['account']['currency'] == 'USD' &&
        (google
            ? m['account']['status'] == 'ENABLED' &&
                  m['account']['manager'] == false &&
                  m['account']['test_account'] == false &&
                  m['root_id'] != null &&
                  (m['login_customer_id'] == null
                      ? m['account']['id'] == m['root_id']
                      : m['login_customer_id'] == m['root_id'])
            : m['account']['status'] == 1 && m['page'] != null);
  }

  final checks = r['checks'];
  var valid =
      r['source'] == (google ? 'google_setup' : 'meta_setup') &&
      r['funnel_id'] == funnel &&
      ...
      expectedChecks.keys.every((k) => checks[k] is bool) &&
  ...
  if (!valid) {
    throw FunnelException(
      'The $label setup review could not be read. Refresh it before continuing.',
    );
  }
  return r;
}

class FunnelCampaignPreparation extends StatefulWidget {
  const FunnelCampaignPreparation({
    super.key,
  ...
  final CampaignSetupPlatform platform;
  final FunnelClient client;
  final String funnelId, campaignId;
  final ValueListenable<int>? scope;
  @override
  State<FunnelCampaignPreparation> createState() =>
      _FunnelCampaignPreparationState();
}

class _FunnelCampaignPreparationState extends State<FunnelCampaignPreparation> {
  Map<String, dynamic>? _data;
  bool _busy = false, _checked = false;
  int _generation = 0;
  String? _error, _unavailable, _message;
  bool get _google => widget.platform == CampaignSetupPlatform.google;
  String get _label => _google ? 'Google Ads' : 'Meta';
  String get _identityKey => _google ? 'google_ads' : 'meta';
  String get _confirmation => _google
  ...
  String get _path =>
      '/${widget.funnelId}/campaigns/${widget.campaignId}/${_google ? 'google' : 'meta'}-setup';
  ...
  void _deny() =>
      _invalidate('Sign in with Enterprise access to review $_label setup.');
  void _scopeChanged() => _invalidate(
    'The campaign workspace changed. Close this review and open it again.',
  );
  @override
  void didUpdateWidget(covariant FunnelCampaignPreparation oldWidget) {
  ...
  void _apply(Map<String, dynamic> r, int g) {
    ...
    final data = validateCampaignPreparation(
      r,
      widget.funnelId,
      widget.campaignId,
        ...
        if (_current(g)) {
          setState(
            () => _error = e is FunnelException
                ? e.message
                : '$_label setup could not complete this request. Try again.',
          );
        }
      }
    } finally {
      ...
      if (_current(g)) {
        setState(
          () => _message = action == 'review'
              ? '$_label setup review saved.'
              : 'Saved $_label setup review cleared.',
        );
      }
    }, recover: true);
  }
  ...
  Widget _snapshot(Map s) {
    ...
    return Column(
      ...
      children: [
        ...
        _gap(18),
        Text(
          _google ? 'Google ad account and access' : 'Meta account and Page',
          style: TextStyle(fontWeight: FontWeight.w700),
        ),
        _gap(8),
        Text(
        ...
        if (_google) ...[
          Text(
            m['root_id'] == null
                ? 'No access account selected'
                : 'Access account: ${m['root_name'] ?? ''} · ${m['root_id']}',
          ),
          _gap(8),
          _caption(
            m['root_id'] == null
                ? 'Refresh Google access accounts to choose how to connect.'
                : m['login_customer_id'] == null
                ? 'Direct account access'
                : 'Manager access: ${m['login_customer_id']}',
          ),
          if (a?['test_account'] == true)
            _caption('Test account · setup review unavailable'),
        ] else
          Text(
            page == null
        ...
        _caption(
          _google
              ? 'Account details come from your last Google refresh. This review does not recheck permission with Google.'
  ...
  Widget build(BuildContext context) {
    final r = _data;
    return AlertDialog(
      title: Text('$_label setup review'),
      content: SizedBox(
        width: 760,
          ...
          child: Column(
            ...
            children: [
              ...
              if (_unavailable == null) ...[
                _caption(
                  _google
                      ? 'Review the campaign plan, landing page, ad account and access account together. Ad creation and spending remain unavailable.'
                ...
                if (r != null) ...[
                  ...
                  _gap(),
                  for (final entry in setupChecks(widget.platform).entries)
                    Padding(
        ...
        if (_unavailable == null) ...[
          ...
          FilledButton(
            onPressed:
                ...
                ? () => _save('review')
                : null,
            child: Text('Save $_label setup review'),
          ),
        ],
      ],
    );

...
const googleCampaignLinkBoundary =
    'This is your reporting association. Inquiry tags are visitor-supplied, so they do not prove Google conversions. The association covers the whole selected period, including dates before it was saved. The link does not certify that the plan matches the provider campaign. No ads, spending limits or publishing permissions are changed.';
const _invalid = FunnelException(
  'The Google campaign link response could not be verified. Refresh it.',
  503,
);
...
bool _accessContext(dynamic root, dynamic login, dynamic account) =>
    _customer(root) &&
    (login == root ||
        (login == null && account is Map && account['id'] == root));
...
bool _account(dynamic a) =>
    ...
    ['ENABLED', 'CANCELED', 'SUSPENDED', 'CLOSED'].contains(a['status']) &&
    a['manager'] is bool &&
...
bool _link(dynamic l) =>
    l is Map &&
    _active(l['account']) &&
    _accessContext(l['root_id'], l['login_customer_id'], l['account']) &&
...
Map<String, dynamic> validateGoogleCampaignLink(
  Map<String, dynamic> d,
  String funnel,
  String campaign,
) {
  if (d['source'] != 'google_campaign_link' ||
      d['funnel_id'] != funnel ||
      ...
      !_stamp(d['checked_at']) ||
      (d['root_id'] != null && !_customer(d['root_id'])) ||
      (d['login_customer_id'] != null && !_customer(d['login_customer_id'])) ||
  ...
  if (d['link_current'] == true &&
      (d['link'] == null ||
          ...
          d['link']['login_customer_id'] != d['login_customer_id'] ||
          [
            'id',
            'currency',
            'timezone',
...
void _reportContext(
  ...
  if (r['source'] != source ||
      ...
      r['root_id'] != d['root_id'] ||
      r['login_customer_id'] != d['login_customer_id'] ||
...
Map<String, dynamic> validateGoogleCampaignChoices(
  Map<String, dynamic> r,
  Map<String, dynamic> d,
  int days,
) {
  _reportContext(r, d, 'google_campaign_choices', days);
  if (d['lookup_ready'] != true ||
      r['choices'] is! List ||
      (r['choices'] as List).length > 500) {
    throw _invalid;
  }
  final ids = <String>{};
...
Map<String, dynamic> validateGoogleLinkedPerformance(
  Map<String, dynamic> r,
  Map<String, dynamic> d,
  int days,
) {
  _reportContext(r, d, 'google_linked_performance', days);
...
String googleLinkedSummary(
  Map<String, dynamic> d,
  Map<String, dynamic>? report,
) {
  final out = StringBuffer(
    'KORLIX GOOGLE CAMPAIGN ASSOCIATION\nPlan: ${d['campaign_name']}\nChecked: ${d['checked_at']}\n',
  );
  final l = d['link'];
  if (l == null) {
    out.writeln('No saved Google campaign association.');
  } else {
    out.writeln(
      'Saved Google campaign: ${l['provider_campaign_name']} (${l['provider_campaign_id']})\nSaved account: ${l['account']['name']} (${l['account']['id']})\nAccess account: ${l['root_id']}\nManager header: ${l['login_customer_id'] ?? 'Direct access'}\nTest account: ${l['account']['test_account']}\nLinked: ${l['linked_at']}\nReporting available at check time: ${d['report_ready']}',
    );
  }
  if (report != null) {
    ...
    if (row == null) {
      out.writeln(
        'Google returned no row for the linked campaign. Spend, clicks and impressions are unknown for this report.',
      );
    } else {
      out.writeln(
        'Current Google name: ${row['campaign_name']}\nGoogle status/channel: ${row['status']} / ${row['channel']}\nGoogle spend: ${metaMoney(r['account']['currency'], row['spend'])}\nGoogle clicks: ${row['clicks']}\nGoogle impressions: ${row['impressions']}',
      );
    }
    out.writeln(
  ...
  out.writeln(
    'Provider totals can be revised. No currency conversion, cost per conversion, revenue or ROAS is inferred.\n$googleCampaignLinkBoundary',
  );
  return out.toString();
}

class FunnelGoogleCampaignLink extends StatefulWidget {
  const FunnelGoogleCampaignLink({
    super.key,
  ...
  @override
  State<FunnelGoogleCampaignLink> createState() =>
      _FunnelGoogleCampaignLinkState();
}

class _FunnelGoogleCampaignLinkState extends State<FunnelGoogleCampaignLink> {
  ...
  String get _path =>
      '/${widget.funnelId}/campaigns/${widget.campaignId}/google-link';
  ...
  @override
  void didUpdateWidget(covariant FunnelGoogleCampaignLink old) {
  ...
  Future<void> _run(String action, [Map<String, dynamic>? body]) async {
    ...
    try {
      ...
      if (action == 'campaigns') {
        final clean = validateGoogleCampaignChoices(r, d!, days);
        setState(() => _choices = clean);
      } else if (action == 'performance') {
        final clean = validateGoogleLinkedPerformance(r, d!, days);
        setState(() => _report = clean);
      } else {
        final clean = validateGoogleCampaignLink(r, funnel, campaign);
        setState(() {
          _data = clean;
          _message = action == 'save'
              ? 'Google campaign association saved.'
              : action == 'clear'
  ...
  Future<void> _copy() async {
    ...
    try {
      await Clipboard.setData(
        ClipboardData(text: googleLinkedSummary(d, _report)),
      );
      if (_current(g)) {
  ...
  Widget _saved(Map d) {
    ...
    if (l == null) {
      return _caption(
        'No Google campaign is linked to this plan. Load a reported campaign list to choose one.',
      );
    }
    return _card(
      Column(
        ...
        children: [
          ...
          _caption('Google campaign ID: ${l['provider_campaign_id']}'),
          if (l['account']['test_account'] == true)
            _caption('Saved Google test account · not live ad delivery.'),
          _caption(
            'Saved access account: ${l['root_id']} · Manager: ${l['login_customer_id'] ?? 'Direct access'}',
          ),
          _caption(
            'Saved account: ${l['account']['name']} · ${l['account']['id']}',
          ),
          _caption(
            'Saved ${l['linked_at']}. The label is a snapshot; Google may rename the campaign.',
          ),
          if (d['report_ready'] != true)
            _caption(
  ...
  Widget _selection() {
      ...
      return _card(
        Column(
          ...
          children: [
            ...
            _caption('Google campaign ID: ${chosen['provider_campaign_id']}'),
            _caption(
              'Account: ${_choices!['account']['name']} · ${_choices!['account']['id']}',
            ),
            _gap(),
            _caption(
              'One Google campaign can link to one of your KORLIX plans per account. This replaces any saved association for this plan.',
            ),
            _gap(),
            Wrap(
              spacing: 10,
    ...
    return Column(
      ...
      children: [
        _gap(),
        _caption(
          'Reported campaigns: ${_choices!['range']['from']} through ${_choices!['range']['to']} · ${_choices!['account']['timezone']}. Only campaigns returned by Google for this period are listed.',
        ),
        if (choices.isEmpty) ...[
          _gap(),
          _caption(
            'Google returned no campaigns for this period. Try another period.',
          ),
        ] else ...[
          _gap(),
          TextField(
            ...
            _card(
              Column(
                ...
                children: [
                  ...
                  _caption(
                    'Google campaign ID: ${row['provider_campaign_id']}',
                  ),
                  _gap(),
                  OutlinedButton(
                    key: ValueKey(
                      'google-link-choice-${row['provider_campaign_id']}',
                    ),
                    onPressed: _busy
                        ? null
                        : () => setState(
      ...
      children: [
        ...
        _card(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const WfBadge('GOOGLE REPORTED'),
              _gap(),
              if (row == null)
                _caption(
                  'Google returned no row for this campaign. Spend, clicks and impressions are unknown for this report.',
                )
              else ...[
                SelectableText('${row['campaign_name']}'),
                _caption('Google campaign ID: ${row['campaign_id']}'),
                _caption(
                  'Google status: ${row['status']} · Channel: ${row['channel']}',
                ),
                _gap(),
                Wrap(
                  spacing: 24,
                  ...
                  children: [
                    ...
                    Text('${metaCount(row['clicks'])} clicks'),
        ...
        _card(
          Column(
            ...
            children: [
              ...
              _caption(
                'Matches this plan’s tracking tag and Google source. These are KORLIX submissions, not verified Google conversions.',
              ),
            ],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: !_busy,
      child: AlertDialog(
        title: const Text('Linked Google campaign'),
        content: SizedBox(
          width: 800,
            ...
            child: Column(
              ...
              children: [
                if (_unavailable != null)
                  ...
                  _caption(googleCampaignLinkBoundary),
                  _gap(),
                  ...
                  if (d != null) ...[
                    ...
                    if (d['configured'] != true)
                      _caption(
                        'Campaign lookup will be available after Google platform setup.',
                      )
                    else if (d['account'] == null)
                      _caption(
                        'Connect Google and select an ad account in Ads workspace.',
                      )
                    else
                      _caption(
                    ...
                    if (d['account'] != null) ...[
                      _caption(
                        'Access account: ${d['root_id'] ?? 'Not selected'} · Manager: ${d['login_customer_id'] ?? 'Direct access'}',
                      ),
                      if (d['account']['test_account'] == true)
                        _caption('Google test account · not live ad delivery.'),
                    ],
                    _gap(),
                      ...
                      children: [
                        OutlinedButton(
                          onPressed: _busy || d['lookup_ready'] != true
                              ? null
                              : () => _run('campaigns'),
                          child: const Text('Load Google campaigns'),
                        ),
                        FilledButton(

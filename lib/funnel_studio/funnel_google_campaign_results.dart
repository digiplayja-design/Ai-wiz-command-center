...
String googleCampaignLabel(String value) => value
    .toLowerCase()
    .split('_')
    .map((word) => '${word[0].toUpperCase()}${word.substring(1)}')
    .join(' ');

int compareGoogleCampaigns(Map a, Map b, String sort) {
  final int order;
  switch (sort) {
    case 'spend':
...
class FunnelGoogleCampaignResults extends StatefulWidget {
  const FunnelGoogleCampaignResults({
    super.key,
    required this.rows,
  ...
  @override
  State<FunnelGoogleCampaignResults> createState() =>
      _FunnelGoogleCampaignResultsState();
}

class _FunnelGoogleCampaignResultsState
    extends State<FunnelGoogleCampaignResults> {
  ...
  Widget _campaign(Map row) => Container(
    key: ValueKey('google-campaign-${row['campaign_id']}'),
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 6),
        _caption(
          'Current status: ${googleCampaignLabel(row['status'])} · Channel: ${googleCampaignLabel(row['channel'])}',
        ),
  ...
  Widget build(BuildContext context) {
    final matches =
        widget.rows
            ...
            .toList()
          ..sort((a, b) => compareGoogleCampaigns(a, b, _sort));
    ...
    return Material(
      ...
      child: Column(
        ...
        children: [
          const Text(
            'Google campaign comparison',
            style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700),
          ),
          const SizedBox(height: 12),
          TextField(
            key: const ValueKey('google-campaign-search'),
            controller: _search,
            maxLength: 100,
          ...
          DropdownButtonFormField<String>(
            key: const ValueKey('google-campaign-sort'),
            initialValue: _sort,
            isExpanded: true,
            ...
            items: const [
              ...
              DropdownMenuItem(value: 'clicks', child: Text('Most clicks')),
          ...
          if (matches.isEmpty) ...[
            ...
            if (matches.length > _pageSize) ...[
              ...
              Wrap(
                ...
                children: [
                  OutlinedButton.icon(
                    key: const ValueKey('google-campaign-prev'),
                    onPressed: _page == 0
                        ? null
                        : () => setState(() => _page--),
                  ...
                  OutlinedButton.icon(
                    key: const ValueKey('google-campaign-next'),

...
bool googleRadiusLabelValid(dynamic v) =>
    v is String &&
    v.isNotEmpty &&
    v.runes.length <= 80 &&
    !v.startsWith(' ') &&
...
bool googleRadiusValid(dynamic v) {
  bool integer(dynamic n, int lo, int hi) => n is int && n >= lo && n <= hi;
  return v is Map &&
      v.length == 4 &&
      googleRadiusLabelValid(v['label']) &&
      integer(v['latitude_micro'], -90000000, 90000000) &&
      integer(v['longitude_micro'], -180000000, 180000000) &&
      integer(v['radius_meters'], 1000, 200000);
}

bool googleRadiiValid(dynamic v) =>
    v is List &&
    v.length <= 10 &&
    v.every(googleRadiusValid) &&
    v
            .map(
              (r) =>
...
String googleRadiusSummary(Map assets) {
  final radii = assets['proximities'];
...
class GoogleRadiusFields extends StatelessWidget {
  ...
  Widget _field(
    ...
    child: TextFormField(
      ...
      onChanged: (v) => onChange(
        area,
        key,
        googleRadiusNumber(v, decimals, minimum, maximum),
      ),
    ),
  );
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  errorMaxLines: 3,
                  errorText: googleRadiusLabelValid(area['label'])
                      ? null
              ...
              _field(
                ...
                'radius_meters',
                'Radius (km)',
                '1 to 200 km; up to 3 decimal places.',
                3,
                1000,
                200000,
              ),
      ...
      if (!googleRadiiValid(areas) && areas.every(googleRadiusValid))
        const Padding(

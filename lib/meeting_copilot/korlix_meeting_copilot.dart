                      ...
                      if (wide)
                        Row(
                          ...
                          children: <Widget>[
                            ...
                            Expanded(
                              flex: 6,
                              child: _TranscriptPanel(state: state, capture: capture),
                            ),
                          ],
                        )
                      else ...<Widget>[
...
class _Header extends StatelessWidget {
  const _Header({required this.korlixLogo, required this.state, this.statusLabel});
      ...
      child: Padding(
        ...
        child: Wrap(
          ...
          children: <Widget>[
            ...
            Semantics(
              label: 'Nova status: ${statusLabel ?? state.status.label}',
              child: Container(
    ...
    return _Panel(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(
            capture == null ? state.meetingTitle : (capture!.meetingUuid == null ? 'Select a meeting' : 'Meeting session selected'),
          ...
          Text(
            capture != null
                ? (capture!.activeSeconds == null ? 'Active processing time unconfirmed' : '${capture!.activeSeconds}s last confirmed active processing')
                : '${state.participantCount} participants • ${state.activeSeconds}s active processing',
            textAlign: TextAlign.center,
            style: const TextStyle(
...
class _TranscriptPanel extends StatelessWidget {
  const _TranscriptPanel({required this.state, this.capture});
  final K135zCaptureController? capture;
  ...
  Widget build(BuildContext context) {
    ...
    if (live != null) return _Panel(title:'Live Transcript Preview', child:Column(
      crossAxisAlignment:CrossAxisAlignment.start, children:[
        ...
        if (live.transcriptTruncated) const Text('Older captions are outside this preview.',
          style:TextStyle(color:KorlixMeetingCopilotScreen._mutedText)),
        for (final line in live.transcriptLines) Padding(padding:const EdgeInsets.only(top:10),
          child:Text('${line.speaker.isEmpty ? "Speaker" : line.speaker}: ${line.text}',
    ...
    return _Panel(
      ...
      child: lines.isEmpty
          ? _EmptyState(
              ...
              message: 'No transcript is being collected. Host approval and '

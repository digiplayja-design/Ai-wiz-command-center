begin;
-- The private HTTP boundary verifies each choice against the pinned Google catalog.
-- SQL independently enforces shape, known country, limits and mutually exclusive modes.
-- Catalog names live inside saved assets so historical reviews remain self-contained.
create function public.korlix_google_location_valid_v1(v jsonb)
returns boolean language plpgsql immutable security invoker set search_path=public,pg_temp as $$
begin
  if v is null or jsonb_typeof(v)<>'object' or not(v ?& array['id','name','country','type']) or (v-array['id','name','country','type'])<>'{}'::jsonb then return false; end if;
  if jsonb_typeof(v->'id')<>'string' or (v->>'id') !~ '^[1-9][0-9]{0,9}$' or jsonb_typeof(v->'name')<>'string' or char_length(v->>'name') not between 1 and 300 or btrim(v->>'name')<>v->>'name' or (v->>'name') ~ ('[[:cntrl:]<>'||chr(127)||'-'||chr(159)||chr(8232)||chr(8233)||']') then return false; end if;
  if jsonb_typeof(v->'country')<>'string' or not exists(select 1 from jsonb_array_elements(public.korlix_google_targeting_catalog_v1()->'countries') x where x->>'code'=v->>'country') then return false; end if;
  if jsonb_typeof(v->'type')<>'string' or v->>'type' not in ('City','State','Province','Region','Department','Canton','Governorate','Prefecture','Autonomous Community','Division','Territory','Union Territory','Okrug') then return false; end if;
  return true;
end $$;
revoke all on function public.korlix_google_location_valid_v1(jsonb) from public,anon,authenticated;
grant execute on function public.korlix_google_location_valid_v1(jsonb) to service_role;
...
begin
  if v is null or jsonb_typeof(v)<>'object' or not(v ?& array['countries','excluded_countries','content_languages','location_mode','bidding']) or (v-array['countries','excluded_countries','content_languages','location_mode','bidding','proximities','geo_locations'])<>'{}'::jsonb then return false; end if;
  ...
  if v ? 'geo_locations' then
    areas:=v->'geo_locations';
    if jsonb_typeof(areas)<>'array' or jsonb_array_length(v->'countries')>0 or v ? 'proximities' then return false; end if;
    if jsonb_array_length(areas)>20 then return false; end if;
    if exists(select 1 from jsonb_array_elements(areas) x where not public.korlix_google_location_valid_v1(x) or v->'excluded_countries' ? (x->>'country')) then return false; end if;
    if (select count(*)<>count(distinct x->>'id') from jsonb_array_elements(areas) x) then return false; end if;
  end if;
  return true;
end $$;
...
begin
  ...
  if p_action='save' then
    ...
    if not public.korlix_google_targeting_valid_v1(p_data->'assets') then raise exception 'Check countries, cities/regions or radius areas, exclusions, content languages, location reach and bidding preference.'; end if;
  ...
  review_checks:=jsonb_build_object(
    ...
    'complete_choices',(jsonb_array_length(assets->'countries')>0 or jsonb_array_length(coalesce(assets->'proximities','[]'::jsonb))>0 or jsonb_array_length(coalesce(assets->'geo_locations','[]'::jsonb))>0) and jsonb_array_length(assets->'content_languages')>0 and assets->>'location_mode'<>'undecided' and assets->>'bidding'<>'undecided',
  ...
  return jsonb_build_object('source','google_targeting_draft','funnel_id',f.id,'campaign_id',c.id,'version',coalesce(saved.version,0),'fingerprint',fingerprint,'context',context,'saved_context',saved.context_snapshot,'assets',assets,'updated_at',saved.updated_at,'draft_current',coalesce(saved.context_fingerprint=fingerprint,false),'editable',c.state<>'archived','draft_complete',(jsonb_array_length(assets->'countries')>0 or jsonb_array_length(coalesce(assets->'proximities','[]'::jsonb))>0 or jsonb_array_length(coalesce(assets->'geo_locations','[]'::jsonb))>0) and jsonb_array_length(assets->'content_languages')>0 and assets->>'location_mode'<>'undecided' and assets->>'bidding'<>'undecided','catalog',catalog,'saved_labels',saved.labels_snapshot,'ad_publishing_ready',false,'radius_supported',true,'locations_supported',true,'location_catalog_version','google-locations-2026-08-12','draft_revision',coalesce(saved.draft_revision,0),'review_fingerprint',current_review_fingerprint,'review_checks',review_checks,'review_ready',review_ready,'review_current',coalesce(review_ready and saved.review_fingerprint=current_review_fingerprint,false),'reviewed_at',saved.reviewed_at,'reviewed_snapshot',saved.reviewed_snapshot);
end $$;

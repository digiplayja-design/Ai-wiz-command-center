begin;
-- Meta IDs and names are verified at the private HTTP boundary with scoped,
-- expiring search receipts. Trusted service callers must preserve that check.
-- Existing countries/radii and review fingerprints are unchanged.
create function public.korlix_meta_location_valid_v1(v jsonb)
returns boolean language plpgsql immutable security invoker set search_path=public,pg_temp as $$
declare k text;
begin
  if v is null or jsonb_typeof(v)<>'object' or not(v ?& array['key','name','country','type','region']) or (v-array['key','name','country','type','region'])<>'{}'::jsonb then return false; end if;
  if jsonb_typeof(v->'key')<>'string' or (v->>'key') !~ '^[0-9]{1,40}$' then return false; end if;
  foreach k in array array['name','region'] loop
    if jsonb_typeof(v->k)<>'string' or char_length(v->>k) not between (case when k='name' then 1 else 0 end) and 200 or btrim(v->>k)<>v->>k or (v->>k) ~ ('[[:cntrl:]<>'||chr(127)||'-'||chr(159)||chr(8232)||chr(8233)||']') then return false; end if;
  end loop;
  if jsonb_typeof(v->'country')<>'string' or not exists(select 1 from jsonb_array_elements(public.korlix_meta_targeting_catalog_v1()->'countries') x where x->>'code'=v->>'country') then return false; end if;
  return jsonb_typeof(v->'type')='string' and v->>'type' in ('city','region');
end $$;
revoke all on function public.korlix_meta_location_valid_v1(jsonb) from public,anon,authenticated;
grant execute on function public.korlix_meta_location_valid_v1(jsonb) to service_role;
...
begin
  if v is null or jsonb_typeof(v)<>'object' or not(v ?& array['countries','age_min','age_max','placements','categories']) or (v-array['countries','age_min','age_max','placements','categories','custom_locations','geo_locations'])<>'{}'::jsonb then return false; end if;
  a:=v->'countries';
  ...
  if v ? 'geo_locations' then
    if jsonb_typeof(v->'geo_locations')<>'array' or jsonb_array_length(v->'countries')>0 or v ? 'custom_locations' then return false; end if;
    if jsonb_array_length(v->'geo_locations')>20 then return false; end if;
    if exists(select 1 from jsonb_array_elements(v->'geo_locations') x where not public.korlix_meta_location_valid_v1(x)) then return false; end if;
    if (select count(*)<>count(distinct jsonb_build_array(x->>'type',x->>'key')) from jsonb_array_elements(v->'geo_locations') x) then return false; end if;
  end if;
...
begin
  ...
  if p_action='save' then
    ...
    if not public.korlix_meta_targeting_valid_v1(p_data->'assets') then raise exception 'Check countries, cities/regions or radius areas, draft ages, categories and placement preference.'; end if;
  ...
  checks:=jsonb_build_object('saved_draft',saved.campaign_id is not null,'complete_choices',(jsonb_array_length(assets->'countries')>0 or ((jsonb_array_length(coalesce(assets->'custom_locations','[]'::jsonb))>0 or jsonb_array_length(coalesce(assets->'geo_locations','[]'::jsonb))>0) and assets->'categories' ? 'NONE')) and assets->>'placements'<>'undecided' and not(assets->'categories' ? 'UNDECIDED'),'current_context',coalesce(saved.context_fingerprint=fingerprint,false),'page_published',setup->'checks'->'page_published','plan_reviewed',setup->'checks'->'plan_reviewed','creative_saved',(creative->>'version')::int>0,'creative_complete',creative->'creative_complete','creative_current',creative->'draft_current');
  ...
  return jsonb_build_object('source','meta_targeting_draft','funnel_id',p_funnel,'campaign_id',c.id,'version',coalesce(saved.version,0),'fingerprint',fingerprint,'creative',creative,'saved_context',saved.context_snapshot,'assets',assets,'updated_at',saved.updated_at,'draft_current',coalesce(saved.context_fingerprint=fingerprint,false),'editable',c.state<>'archived','draft_complete',checks->'complete_choices','catalog',catalog,'saved_labels',saved.labels_snapshot,'ad_publishing_ready',false,'radius_supported',true,'locations_supported',true,'location_lookup_ready',coalesce(c.state<>'archived' and setup->'checks'->'meta_configured'='true'::jsonb and setup->'checks'->'meta_connected'='true'::jsonb and setup->'checks'->'account_selected'='true'::jsonb and setup->'checks'->'account_active'='true'::jsonb,false),'draft_revision',coalesce(saved.draft_revision,0),'review_fingerprint',review_hash,'review_checks',checks,'review_ready',ready,'review_current',coalesce(ready and saved.review_fingerprint=review_hash,false),'reviewed_at',saved.reviewed_at,'reviewed_snapshot',saved.reviewed_snapshot);
end $$;

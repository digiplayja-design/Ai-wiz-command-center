-- K189: version new Search drafts without changing historical attempts.
begin;

create or replace function public.korlix_funnel_google_create_v1(p_actor uuid,p_action text,p_funnel uuid,p_data jsonb default '{}')
...
begin
  ...
  draft:=jsonb_build_object('plan',prep->'setup'->'current_snapshot'->'campaign','page',prep->'setup'->'current_snapshot'->'landing_page','identity',prep->'setup'->'current_snapshot'->'google_ads','creative',prep->'creative'->'assets','keywords',prep->'keywords'->'assets','targeting',prep->'targeting'->'assets','search_language_mode','automatic_from_creative_v1');

...
import 'package:ai_wiz_command_center/funnel_studio/funnel_meta_campaign_link.dart';
...
Map<String, dynamic> choices(Map d, {int count = 2}) =>
    common(d, 'meta_campaign_choices')
      ..['choices'] = [
...
}) => common(d, 'meta_linked_performance')
  ..addAll(
    clone({
      'link': d['link'],
      'row': missing
...
Future<void> choose(WidgetTester t, String id) async {
  final f = find.byKey(ValueKey('meta-link-choice-$id'));
...
void main() {
  ...
  test(
    ...
    () {
      expect(validateMetaCampaignLink(fixture(), 'f', 'c')['version'], 1);
      expect(
        validateMetaCampaignLink(
      ...
      for (final mutate in <void Function(Map<String, dynamic>)>[
        ...
        (d) => d['link']['account']['id'] = 'act_999',
        (d) => d['account']['status'] = 2,
        (d) => d['editable'] = false,
        (d) => d['version'] = 0,
        ...
        mutate(d);
        expect(
          () => validateMetaCampaignLink(d, 'f', 'c'),
  ...
  test(
    ...
    () {
      final d = fixture();
      expect(
        validateMetaCampaignChoices(choices(d), d, 7)['choices'],
        hasLength(2),
      );
      for (final mutate in <void Function(Map<String, dynamic>)>[
        ...
        mutate(r);
        expect(
          () => validateMetaCampaignChoices(r, d, 7),
          throwsA(isA<FunnelException>()),
        );
      }
    },
  ...
  test(
    ...
    () {
      final d = fixture();
      expect(
        validateMetaLinkedPerformance(report(d), d, 7)['row']['spend'],
        '0.123456',
      );
      for (final mutate in <void Function(Map<String, dynamic>)>[
        ...
        mutate(r);
        expect(
          () => validateMetaLinkedPerformance(r, d, 7),
          throwsA(isA<FunnelException>()),
        );
      }
    },
  ...
  test(
    ...
    () {
      ...
      final s = metaLinkedSummary(d, r);
      expect(s, contains('KWD 0.123456'));
      expect(s, contains('2'));
      expect(s, contains('Renamed in Meta'));
      expect(s, contains('2026-09-16T04:00:00Z inclusive'));
      expect(s, contains(metaCampaignLinkBoundary));
      expect(s, contains('No currency conversion'));
      expect(s, isNot(contains(proof)));
      final missing = validateMetaLinkedPerformance(
        report(d, missing: true),
        d,
        7,
      );
      expect(metaLinkedSummary(d, missing), contains('unknown'));
      final zero = validateMetaLinkedPerformance(report(d, spend: '0'), d, 7);
      expect(metaLinkedSummary(d, zero), contains('Meta spend: KWD 0\n'));
      expect(metaLinkedSummary(d, zero), isNot(contains('unknown')));
    },
  );
  for (final size in [
    ...
    testWidgets('Choice and linked report fit ${size.width}', (t) async {
      ...
      await tap(t, 'Load Meta campaigns');
      await choose(t, '12');
      expect(find.text('Meta campaign ID: 12'), findsOneWidget);
      ...
      expect(find.text('META REPORTED'), findsOneWidget);
  ...
  testWidgets(
    ...
    (t) async {
      ...
      expect(find.text('Meta campaign association saved.'), findsOneWidget);
      expect(find.text('Meta campaign ID: 12'), findsOneWidget);
  ...
  testWidgets(
    ...
    (t) async {
      ...
      expect(copied, contains(metaCampaignLinkBoundary));
        ...
        expect(
          t
              .widget<OutlinedButton>(
                find.widgetWithText(OutlinedButton, 'Load Meta campaigns'),
              )
              .onPressed,
          isNull,
        );
  ...
  testWidgets('Campaign search matches ID and paginates bounded results', (
    ...
    await show(t, c);
    await tap(t, 'Load Meta campaigns');
    expect(find.byKey(const ValueKey('meta-link-choice-30')), findsOneWidget);
    expect(find.byKey(const ValueKey('meta-link-choice-31')), findsNothing);
    await tap(t, 'Next campaigns');
    expect(find.byKey(const ValueKey('meta-link-choice-31')), findsOneWidget);
    expect(find.byKey(const ValueKey('meta-link-choice-11')), findsNothing);

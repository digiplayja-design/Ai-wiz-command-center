...
void gate6cHubTests() {
  test('Gate6C hub passes selected catalog agent and current-client binding', () {
    final s = r('lib/live_convo/korlix_live_convo_agent_sheet.dart');
    expect(s, contains('_catalog.agentById(_selectedAgentId)'));
    expect(s, contains('agentId: selected.id'));
    expect(s, contains('arguments: launch'));
    expect(s, contains('headersBuilder: selectedClient.headersBuilder'));
    expect(s, contains('identical(widget.client, selectedClient)'));
    expect(s, contains('_loading || _busy || selected == null || !selected.active'));
  });
}
